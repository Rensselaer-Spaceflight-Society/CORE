...
**New to CORE? Start with the [first-meeting workspace](workspaces/README.md).** Pick your sub-team and task code; each card has a small introductory task and a place to record findings. No coding or Git installation is required for the first meeting. Each card also has an optional [guided Python worksheet](workspaces/python/README.md) that teaches the topic and collects your findings; it needs no experience, and no install to take part.
...
| `workspaces/python/` | Beginner Python worksheets, one per task code. Teaching only: they import no engine module, read no configuration and set no design value |
